  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (priority !== undefined) {
        newPriority =
          priority.trim().charAt(0).toUpperCase() + priority.trim().slice(1);
        if (
          newPriority !== "Low" &&
          newPriority !== "Medium" &&
          newPriority !== "High"
...
const getATask = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const conn = await pool.connect();
      const sql =
        "SELECT * from Task where todo_id = ($1) and task_id = ($2) and user_id = ($3);";
...
const editATask = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const conn = await pool.connect();
      const sql =
        "UPDATE Task SET name = COALESCE($1, name), checked = COALESCE($2, checked), priority = COALESCE($3, priority), deadline = COALESCE($4, deadline), updated_at=now() WHERE todo_id =($5) AND task_id =($6) AND user_id = ($7) RETURNING *;";
      const result = await conn.query(sql, [
        validatedName,
        completed,
        validatedPriority,
...
const deleteATask = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const conn = await pool.connect();
      const sql =
        "DELETE FROM Task WHERE todo_id = ($1) AND task_id = ($2) AND user_id = ($3) RETURNING *;";
      const result = await conn.query(sql, [
        req.params.todo_id,
        req.params.task_id,
        user_id,
...
const reorderATask = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const conn = await pool.connect();
      const sql =
        "SELECT * FROM Task WHERE user_id = ($1) AND todo_id = ($2) AND task_id = ($3);";
...
const uploadAFile = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const sql =
        "UPDATE Task SET document = ($1), updated_at = NOW() WHERE task_id = ($2) AND todo_id = ($3) AND user_id = ($4) RETURNING *;";
      const result = await conn.query(sql, [
        fileUpload,
        req.params.task_id,
        req.params.todo_id,
